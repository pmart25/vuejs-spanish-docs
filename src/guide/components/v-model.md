...
Cuando aprendimos sobre los enlaces de entrada de formularios, vimos que `v-model` tiene [modificadores integrados](/guide/essentials/forms#modificadores): `.trim`, `.number` y `.lazy`. En algunos casos, es posible que también quieras que el `v-model` de tu componente de entrada personalizado soporte modificadores personalizados.
