...
Para los usuarios de TSX, se sugiere establecer [jsxImportSource](https://www.typescriptlang.org/tsconfig#jsxImportSource) a `'vue'` en `tsconfig.json` después de actualizar a 3.3, u optar por archivo con `/* @jsxImportSource vue */`. Esto le permitirá optar por el nuevo comportamiento ahora y actualizar sin problemas cuando la versión 3.4 sea lanzada.

Si hay código que depende de la presencia del espacio de nombres global `JSX`, puede conservar el comportamiento global exacto anterior a 3.4 haciendo referencia explícita a `vue/jsx`, que registra el espacio de nombres global `JSX`.
